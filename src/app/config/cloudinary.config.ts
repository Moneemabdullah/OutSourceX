...
export const uploadFileToCloudinary = async (
  ...
  if (!buffer || !fileName) {
    throw new AppError(status.BAD_REQUEST, 'Buffer and file name are required for upload');
  }
  ...
  const uniqueName =
    Math.random().toString(36).substring(2) + '_' + Date.now() + '_' + fileNameWithoutExtension;
